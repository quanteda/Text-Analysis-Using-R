...
# check the first five stopwords from English ISO code list (note that numbers are included)
...
toks_stemming <- tokens(txt)
tokens_wordstem(toks_stemming)
```
...
```{r}
txt <- "During my stay in London I visited the museum and attended a very good concert."
...
Some processing steps are sequence-dependent, for instance when a user removes stopwords and also applies a stemmer.
